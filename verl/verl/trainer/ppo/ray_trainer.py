...
class RayPPOTrainer:
    ...
    def _create_dataloader(self, train_dataset, val_dataset, collate_fn, train_sampler: Optional[Sampler]):
        ...
        total_training_steps = len(self.train_dataloader) * configured_total_epochs

        if self.config.trainer.total_training_steps is not None:
            total_training_steps = int(self.config.trainer.total_training_steps)

        self.total_training_epochs = configured_total_epochs
    ...
    def _log_rollout_data(
        ...
        with marked_timer("dump_rollout_generations", timing_raw, color="green"):
            ...
            for metric_name in ("sequence_entropy", "token_entropy_variance", "sequence_ppl", "sequence_kl"):
                if metric_name in batch.non_tensor_batch:
                    reward_extra_infos_to_dump[metric_name] = np.asarray(
                        batch.non_tensor_batch[metric_name], dtype=np.float64
                    ).tolist()
    ...
    def _balance_batch(self, batch: DataProto, metrics, logging_prefix="global_seqlen", keep_minibatch=False, prompt_minibatch_size=None):
        ...
        if keep_minibatch:
            # ppo_mini_batch_size is configured in prompts; the post-rollout batch contains n sequences per prompt.
            minibatch_size = (
                (prompt_minibatch_size or self.config.actor_rollout_ref.actor.get("ppo_mini_batch_size"))
                * self.config.actor_rollout_ref.rollout.n
            )
            if len(global_seqlen_lst) % minibatch_size != 0:
                raise ValueError(
                    f"Post-rollout batch size {len(global_seqlen_lst)} must be divisible by the logical PPO "
                    f"mini-batch size {minibatch_size} when prompt grouping is enabled"
                )
            if minibatch_size % world_size != 0:
                raise ValueError(
                    f"Logical PPO mini-batch size {minibatch_size} sequences must be divisible by "
                    f"actor world size {world_size} when prompt grouping is enabled"
                )
            ...
            for i in range(minibatch_num):
                ...
                for j, part in enumerate(rearrange_minibatch_lst):
                    part = [x + minibatch_size * i for x in part]
                    part.sort(key=lambda x: (global_seqlen_lst[x], x))
                    global_partition_lst[j].extend(part[::2] + part[1::2][::-1])
        else:
    ...
    def fit(self):
        ...
        for epoch in range(self.total_training_epochs):
            for batch_dict in self.train_dataloader:
                ...
                prompt_filtering_config = self.config.trainer.get("prompt_filtering", None)
                rollout_level_config = self.config.trainer.get("rollout_level_weighting", None)
                prompt_filtering_enabled = bool(prompt_filtering_config and prompt_filtering_config.get("enable", False))
                rollout_level_weighting_enabled = bool(
                    rollout_level_config and rollout_level_config.get("enable", False)
                )
                if (
                    rollout_level_weighting_enabled
                    and self.config.actor_rollout_ref.actor.loss_agg_mode != "seq-mean-token-mean"
                ):
                    raise ValueError("trainer.rollout_level_weighting requires actor.loss_agg_mode=seq-mean-token-mean")
                if rollout_level_weighting_enabled and int(rollout_level_config.get("num_blocks", 8)) != 8:
                    raise ValueError("trainer.rollout_level_weighting.num_blocks must be 8")
                rollout_level_save_every = int(rollout_level_config.get("save_every", 20)) if rollout_level_weighting_enabled else 20
                if rollout_level_weighting_enabled and rollout_level_save_every <= 0:
                    raise ValueError("trainer.rollout_level_weighting.save_every must be positive")
                rollout_level_should_dump = rollout_level_weighting_enabled and self.global_steps % rollout_level_save_every == 0
                if prompt_filtering_enabled:
                    prompt_minibatch_size = int(self.config.actor_rollout_ref.actor.ppo_mini_batch_size)
                    if len(batch) % prompt_minibatch_size != 0:
                        raise ValueError(
                            f"Training prompt batch size {len(batch)} must be divisible by "
                            f"prompt filtering mini-batch size {prompt_minibatch_size}"
                        )
                    grouping_sequence_count = prompt_minibatch_size * int(self.config.actor_rollout_ref.rollout.n)
                    if grouping_sequence_count % self.actor_rollout_wg.world_size != 0:
                        raise ValueError(
                            f"Prompt filtering mini-batch has {grouping_sequence_count} rollout sequences, which must be divisible by "
                            f"actor world size {self.actor_rollout_wg.world_size}"
                        )
                    batch.non_tensor_batch["prompt_filtering_minibatch_id"] = np.arange(len(batch)) // prompt_minibatch_size
                ...
                if prompt_filtering_enabled or rollout_level_weighting_enabled:
                    rollout_n = int(self.config.actor_rollout_ref.rollout.n)
                    gen_batch_output.non_tensor_batch["rollout_id"] = np.tile(
                        np.arange(rollout_n), len(gen_batch_output) // rollout_n
                    )
                    if prompt_filtering_enabled:
                        gen_batch_output.non_tensor_batch["prompt_filtering_rollout_id"] = gen_batch_output.non_tensor_batch["rollout_id"].copy()
                ...
                with marked_timer("step", timing_raw):
                    ...
                    if rollout_level_weighting_enabled:
                        batch.meta_info["calculate_rollout_embedding"] = True
                        batch.meta_info["rollout_embedding_num_blocks"] = 8
                    # Balance the number of valid tokens across DP ranks.
                    # NOTE: This usually changes the order of data in the `batch`,
                    # which won't affect the advantage calculation (since it's based on uid),
                    # but might affect the loss calculation (due to the change of mini-batching).
                    if self.config.trainer.balance_batch or prompt_filtering_enabled:
                        self._balance_batch(
                            batch, metrics=metrics, keep_minibatch=prompt_filtering_enabled,
                            prompt_minibatch_size=prompt_minibatch_size if prompt_filtering_enabled else None,
                        )
                    ...
                    entropys = batch.batch.get("entropys", None)
                    need_rollout_embedding = rollout_level_weighting_enabled and "rollout_embeddings" not in batch.batch
                    if need_recomputation or need_rollout_embedding:
                        # Optimization: Reuse data if available from Distillation Phase
                        if (
                            need_recomputation and "old_log_probs" in batch.batch.keys()
                            and "entropys" in batch.batch.keys() and not need_rollout_embedding
                        ):
                             ...
                             with marked_timer("old_log_prob", timing_raw, color="blue"):
                                 ...
                                 if need_recomputation:
                                     ...
                                     batch.batch["rollout_embeddings"] = old_log_prob.batch["rollout_embeddings"]
                    ...
                    if rollout_level_weighting_enabled:
                        if "rollout_embeddings" not in batch.batch:
                            raise RuntimeError("Current-policy rollout embeddings were not returned by the actor")
                        batch.meta_info.pop("calculate_rollout_embedding", None)
                        batch.meta_info.pop("rollout_embedding_num_blocks", None)

                    if entropys is not None:
                        raw_response_masks = batch.batch["response_mask"]
                        valid_token_counts = raw_response_masks.sum(dim=-1).clamp_min(1)
                        sequence_entropy = (entropys * raw_response_masks).sum(dim=-1) / valid_token_counts
                        batch.non_tensor_batch["sequence_entropy"] = np.asarray(
                            sequence_entropy.detach().cpu().tolist(), dtype=np.float64
                        )
                        token_entropy_variance = ((entropys - sequence_entropy.unsqueeze(-1)).square() * raw_response_masks).sum(dim=-1) / valid_token_counts
                        batch.non_tensor_batch["token_entropy_variance"] = np.asarray(
                            token_entropy_variance.detach().cpu().tolist(), dtype=np.float64
                        )
                        sequence_nll = -(batch.batch["old_log_probs"] * raw_response_masks).sum(
                            dim=-1
                        ) / raw_response_masks.sum(dim=-1).clamp_min(1)
                        sequence_nll = np.asarray(sequence_nll.detach().cpu().tolist(), dtype=np.float64)
                        batch.non_tensor_batch["sequence_ppl"] = np.exp(
                            np.minimum(sequence_nll, np.log(np.finfo(np.float64).max))
                        )
                        response_masks = raw_response_masks
                    ...
                    telemetry_should_dump = False
                    telemetry_entropys = None
                    try:
                        telemetry_config = self.config.trainer.get("rollout_telemetry", None)
                        if telemetry_config and telemetry_config.get("enable", False):
                            if entropys is None:
                                raise RuntimeError(
                                    "actor entropys are unavailable; disable "
                                    "algorithm.rollout_correction.bypass_old_logprob_for_rollout to collect telemetry"
                                )
                            save_every = int(telemetry_config.get("save_every", 1))
                            if save_every <= 0:
                                raise ValueError("trainer.rollout_telemetry.save_every must be positive")
                            if self.global_steps % save_every == 0:
                                telemetry_should_dump = True
                                telemetry_entropys = entropys.detach().cpu()
                    except Exception as error:
                        telemetry_config = None
                        print(
                            f"WARNING: rollout telemetry setup failed at step {self.global_steps}; "
                            f"training will continue without this dump: {error}", flush=True,
                        )
                        traceback.print_exc()
                        metrics["rollout_telemetry/save_failed"] = 1.0
                    ...
                    if need_recomputation and "rollout_log_probs" in batch.batch.keys():
                    ...
                    if "ref_log_prob" in batch.batch.keys():
                        try:
                            telemetry_kl_estimator = telemetry_config.get("kl_estimator", None) if telemetry_config else None
                            if telemetry_kl_estimator:
                                kl_estimator = str(telemetry_kl_estimator)
                            elif self.config.actor_rollout_ref.actor.use_kl_loss:
                                kl_estimator = self.config.actor_rollout_ref.actor.kl_loss_type
                            else:
                                kl_estimator = self.config.algorithm.kl_penalty
                            token_kl = core_algos.kl_penalty(
                                batch.batch["old_log_probs"], batch.batch["ref_log_prob"], kl_penalty=kl_estimator
                            )
                            response_mask_for_kl = batch.batch["response_mask"]
                            sequence_kl = (token_kl * response_mask_for_kl).sum(dim=-1) / response_mask_for_kl.sum(
                                dim=-1
                            ).clamp_min(1)
                            batch.non_tensor_batch["sequence_kl"] = np.asarray(
                                sequence_kl.detach().cpu().tolist(), dtype=np.float64
                            )
                        except Exception as error:
                            token_kl = None
                            kl_estimator = None
                            print(
                                f"WARNING: optional rollout KL telemetry failed at step {self.global_steps}; "
                                f"training will continue without KL telemetry: {error}", flush=True,
                            )
                            traceback.print_exc()
                            metrics["rollout_telemetry/kl_failed"] = 1.0

                    if telemetry_should_dump:
                        try:
                            output_dir = telemetry_config.get("output_dir") or os.path.join(
                                self.config.trainer.default_local_dir, "rollout_telemetry"
                            )
                            if token_kl is None:
                                print(
                                    f"Rollout telemetry step {self.global_steps}: KL plots are unavailable because "
                                    "the reference policy is disabled or KL telemetry failed."
                                )
                            with marked_timer("dump_rollout_telemetry", timing_raw, color="green"):
                                from verl.trainer.ppo.rollout_telemetry import dump_rollout_telemetry_safely

                                telemetry_path = dump_rollout_telemetry_safely(
                                    batch=batch, tokenizer=self.tokenizer, step=self.global_steps,
                                    output_dir=output_dir, config=telemetry_config, entropys=telemetry_entropys,
                                    token_kl=token_kl, kl_estimator=kl_estimator,
                                    token_entropy_weighting_enable=bool(
                                        self.config.actor_rollout_ref.actor.get("token_entropy_weighting_enable", False)
                                    ),
                                    token_entropy_weighting_window_size=int(
                                        self.config.actor_rollout_ref.actor.get("token_entropy_weighting_window_size", 2)
                                    ),
                                    token_entropy_weighting_epsilon=float(
                                        self.config.actor_rollout_ref.actor.get("token_entropy_weighting_epsilon", 1e-6)
                                    ),
                                    expected_rollouts=int(self.config.actor_rollout_ref.rollout.n),
                                )
                            metrics["rollout_telemetry/save_failed"] = float(telemetry_path is None)
                        except Exception as error:
                            print(
                                f"WARNING: rollout telemetry integration failed at step {self.global_steps}; "
                                f"training will continue without this dump: {error}", flush=True,
                            )
                            traceback.print_exc()
                            metrics["rollout_telemetry/save_failed"] = 1.0
                    ...
                    precomputed_prompt_filter = None
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        if prompt_filtering_enabled and rollout_level_weighting_enabled:
                            from verl.trainer.ppo.prompt_filtering import compute_prompt_variance_filter_weights, reduce_binary_outcome_rewards

                            filter_sequence_rewards = reduce_binary_outcome_rewards(
                                token_level_rewards=batch.batch["true_reward_score"], response_mask=batch.batch["response_mask"]
                            )
                            filter_weights, filter_metrics, filter_details = compute_prompt_variance_filter_weights(
                                sequence_rewards=filter_sequence_rewards, prompt_uids=batch.non_tensor_batch["uid"],
                                minibatch_ids=batch.non_tensor_batch["prompt_filtering_minibatch_id"],
                                expected_rollouts=int(self.config.actor_rollout_ref.rollout.n),
                                expected_prompts=int(self.config.actor_rollout_ref.actor.ppo_mini_batch_size),
                                drop_fraction=float(prompt_filtering_config.get("drop_fraction", 0.2)),
                            )
                            batch.batch["prompt_filter_loss_weights"] = filter_weights.to(
                                device=batch.batch["response_mask"].device, dtype=torch.float32
                            )
                            precomputed_prompt_filter = filter_weights, filter_metrics, filter_details, filter_sequence_rewards

                        if rollout_level_weighting_enabled:
                            if "rollout_weight" in batch.batch:
                                raise RuntimeError("rollout_weight was already computed for this rollout batch")
                            if "rollout_embeddings" not in batch.batch:
                                raise RuntimeError(
                                    "Rollout-level weighting requires cached current-policy rollout embeddings"
                                )
                            from verl.trainer.ppo.prompt_filtering import reduce_sequence_rewards
                            from verl.trainer.ppo.rollout_level_weighting import compute_rollout_level_weights, dump_rollout_level_weighting_jsonl

                            outcome_rewards = reduce_sequence_rewards(
                                token_level_rewards=batch.batch["true_reward_score"],
                                response_mask=batch.batch["response_mask"], reward_mode="grpo",
                            )
                            with marked_timer("rollout_level_weighting", timing_raw, color="green"):
                                rollout_weight, rollout_level_metrics, rollout_level_details = compute_rollout_level_weights(
                                    rollout_embeddings=batch.batch.pop("rollout_embeddings"),
                                    sequence_rewards=outcome_rewards, prompt_uids=batch.non_tensor_batch["uid"],
                                    temperature=float(rollout_level_config.get("temperature", 1.0)),
                                    expected_rollouts=int(self.config.actor_rollout_ref.rollout.n),
                                    eligible_mask=batch.batch["prompt_filter_loss_weights"] > 0 if prompt_filtering_enabled else None,
                                )
                            rollout_weight = rollout_weight.to(
                                device=batch.batch["response_mask"].device, dtype=torch.float32
                            )
                            batch.batch["rollout_weight"] = rollout_weight.detach()
                            metrics.update(rollout_level_metrics)
                            if rollout_level_should_dump:
                                output_dir = rollout_level_config.get("output_dir") or os.path.join(
                                    self.config.trainer.default_local_dir, "rollout_level_weighting"
                                )
                                with marked_timer("dump_rollout_level_weighting", timing_raw, color="green"):
                                    filename = dump_rollout_level_weighting_jsonl(
                                        batch=batch, tokenizer=self.tokenizer, sequence_rewards=outcome_rewards,
                                        rollout_weights=rollout_weight, step=self.global_steps, output_dir=output_dir,
                                        details=rollout_level_details,
                                        temperature=float(rollout_level_config.get("temperature", 1.0)),
                                        num_blocks=int(rollout_level_config.get("num_blocks", 8)),
                                        digits=int(rollout_level_config.get("digits", 8)),
                                    )
                                print(f"Dumped rollout-level weighting telemetry to {filename}")
                        ...
                        if prompt_filtering_enabled:
                            from verl.trainer.ppo.prompt_filtering import compute_prompt_variance_filter_weights, dump_prompt_filtering_jsonl, reduce_binary_outcome_rewards

                            sequence_rewards = reduce_binary_outcome_rewards(
                                token_level_rewards=batch.batch["true_reward_score"], response_mask=batch.batch["response_mask"]
                            )
                            filtering_kwargs = {
                                "sequence_rewards": sequence_rewards, "prompt_uids": batch.non_tensor_batch["uid"],
                                "minibatch_ids": batch.non_tensor_batch["prompt_filtering_minibatch_id"],
                                "expected_rollouts": int(self.config.actor_rollout_ref.rollout.n),
                                "expected_prompts": int(self.config.actor_rollout_ref.actor.ppo_mini_batch_size),
                            }
                            if precomputed_prompt_filter is None:
                                relative_weights, filtering_metrics, prompt_details = compute_prompt_variance_filter_weights(
                                    **filtering_kwargs, drop_fraction=float(prompt_filtering_config.get("drop_fraction", 0.2))
                                )
                            else:
                                relative_weights, filtering_metrics, prompt_details, sequence_rewards = precomputed_prompt_filter
                            metrics.update(filtering_metrics)
                            batch.batch["prompt_filter_loss_weights"] = relative_weights.to(
                                device=batch.batch["response_mask"].device, dtype=torch.float32
                            )
                            save_every = int(prompt_filtering_config.get("save_every", 1))
                            if save_every <= 0:
                                raise ValueError("trainer.prompt_filtering.save_every must be positive")
                            if self.global_steps % save_every == 0:
                                output_dir = prompt_filtering_config.get("output_dir") or os.path.join(
                                    self.config.trainer.default_local_dir, "prompt_filtering",
                                )
                                with marked_timer("dump_prompt_filtering", timing_raw, color="green"):
                                    filename = dump_prompt_filtering_jsonl(
                                        ...
                                        rollout_ids=batch.non_tensor_batch["prompt_filtering_rollout_id"],
                                        sample_ids=batch.non_tensor_batch.get("index", None),
                                    )
                                print(f"Dumped prompt filtering telemetry to {filename}")
