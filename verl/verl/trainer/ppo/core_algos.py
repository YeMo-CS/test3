...
PolicyLossFn = Callable[
    [
        ...
        torch.Tensor | None,  # sequence_weights
    ],
...
def agg_loss(
    loss_mat: torch.Tensor, loss_mask: torch.Tensor, loss_agg_mode: str, sequence_weights: torch.Tensor | None = None
):
    """
    ...
    if sequence_weights is not None:
        if loss_agg_mode != "seq-mean-token-mean":
            raise ValueError("sequence_weights are only supported with seq-mean-token-mean")
        if sequence_weights.ndim != 1 or len(sequence_weights) != loss_mat.shape[0]:
            raise ValueError(
                f"sequence_weights must have shape [{loss_mat.shape[0]}], got {tuple(sequence_weights.shape)}"
            )
        sequence_weights = sequence_weights.detach().to(device=loss_mat.device, dtype=loss_mat.dtype)
    ...
    if loss_agg_mode == "token-mean":
        ...
        if sequence_weights is not None:
            seq_losses = seq_losses * sequence_weights
        loss = verl_F.masked_mean(seq_losses, seq_mask)  # seq-mean
...
def compute_policy_loss_vanilla(
    ...
    if format_mask is not None:
        pg_loss = agg_loss(
            loss_mat=pg_losses, loss_mask=response_mask * format_mask.unsqueeze(-1),
            loss_agg_mode=loss_agg_mode, sequence_weights=sequence_weights,
        )
    else:
        pg_loss = agg_loss(
