...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            with adapter_ctx:
                actor_outputs = self.actor.compute_log_prob(data=data, calculate_entropy=True)
                if len(actor_outputs) == 5:
                    output, entropys, topk_ids, topk_log_probs, rollout_embeddings = actor_outputs
                else:
                    output, entropys, topk_ids, topk_log_probs = actor_outputs
                    rollout_embeddings = None
            ...
            if rollout_embeddings is not None:
                tensors["rollout_embeddings"] = rollout_embeddings.detach()
    ...
    def compute_ref_log_prob(self, data: DataProto):
        data.meta_info["calculate_rollout_embedding"] = False
        if self._is_lora:
