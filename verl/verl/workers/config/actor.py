...
class ActorConfig(BaseConfig):
    ...
    Args:
        ...
        token_entropy_weighting_enable (bool): Whether to weight token policy objectives by bidirectional local entropy peaks.
        token_entropy_weighting_window_size (int): Number of available entropy neighbors considered on each side.
        token_entropy_weighting_epsilon (float): Positive floor added to every valid token score before normalization.
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.token_entropy_weighting_window_size, int) or isinstance(self.token_entropy_weighting_window_size, bool) or self.token_entropy_weighting_window_size <= 0:
            raise ValueError(f"token_entropy_weighting_window_size must be a positive integer, got {self.token_entropy_weighting_window_size}")
        if not isinstance(self.token_entropy_weighting_epsilon, (int, float)) or not math.isfinite(self.token_entropy_weighting_epsilon) or self.token_entropy_weighting_epsilon <= 0:
            raise ValueError(f"token_entropy_weighting_epsilon must be positive and finite, got {self.token_entropy_weighting_epsilon}")
