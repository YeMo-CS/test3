...
from verl.trainer.ppo.rollout_level_weighting import pool_response_hidden_states
...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _get_output_embedding_layer(self) -> nn.Module:
        candidates, seen = [self.actor_module], set()
        while candidates:
            ...
            if callable(get_output_embeddings) and get_output_embeddings() is not None:
                return get_output_embeddings()
            if isinstance(getattr(module, "lm_head", None), nn.Module):
                return module.lm_head
            for attribute in ("module", "_fsdp_wrapped_module", "base_model", "model"):
                child = getattr(module, attribute, None)
                if isinstance(child, nn.Module):
                    candidates.append(child)
        raise RuntimeError("Could not locate the policy output-embedding layer needed to capture final hidden states")

    def _forward_micro_batch(
        self, micro_batch, temperature, calculate_entropy=False, top_k=0, student_top_k_ids=None,
        calculate_rollout_embedding=False, rollout_embedding_num_blocks=8,
        ...
        Returns:
            ...
            rollout_embedding: # (bs, num_blocks * hidden_size)
        """
        ...
        with torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            ...
            topk_ids = None
            topk_log_probs = None
            rollout_embedding = None
            captured_hidden_states = []

            def capture_last_hidden_states(_module, inputs):
                captured_hidden_states.append(inputs[0])
            ...
            if self.use_remove_padding:
                ...
                hidden_hook = (
                    self._get_output_embedding_layer().register_forward_pre_hook(capture_last_hidden_states)
                    if calculate_rollout_embedding else None
                )
                try:
                    ...
                    if hidden_hook is not None:
                        hidden_hook.remove()
                if calculate_rollout_embedding and len(captured_hidden_states) != 1:
                    raise RuntimeError(f"Expected one final-hidden-state capture, got {len(captured_hidden_states)}")
                ...
                if self.use_ulysses_sp:
                    ...
                    if calculate_rollout_embedding:
                        captured_hidden_states[0] = gather_outputs_and_unpad(
                            captured_hidden_states[0].squeeze(0), gather_dim=0, unpad_dim=0, padding_size=pad_size
                        )
                ...
                if calculate_rollout_embedding:
                    hidden_states_rmpad = captured_hidden_states[0]
                    if hidden_states_rmpad.ndim == 3:
                        hidden_states_rmpad = hidden_states_rmpad.squeeze(0)
                    full_hidden_states = pad_input(
                        hidden_states=hidden_states_rmpad, indices=indices, batch=batch_size, seqlen=seqlen
                    )
                    response_hidden_states = full_hidden_states[:, -response_length:, :]
                    rollout_embedding = pool_response_hidden_states(
                        response_hidden_states, micro_batch["response_mask"], rollout_embedding_num_blocks
                    )
                ...
                if calculate_rollout_embedding:
                    if len(captured_hidden_states) != 1:
                        raise RuntimeError(f"Expected one final-hidden-state capture, got {len(captured_hidden_states)}")
                    response_hidden_states = captured_hidden_states[0][:, -response_length:, :]
                    rollout_embedding = pool_response_hidden_states(
                        response_hidden_states, micro_batch["response_mask"], rollout_embedding_num_blocks
                    )
            ...
            return entropy, log_probs, topk_ids, topk_log_probs, rollout_embedding
    ...
    def compute_log_prob(self, data: DataProto, calculate_entropy=False) -> torch.Tensor:
        ...
        use_dynamic_bsz = data.meta_info["use_dynamic_bsz"]
        has_multi_modal_inputs = "multi_modal_inputs" in data.non_tensor_batch.keys()
        calculate_rollout_embedding = bool(data.meta_info.get("calculate_rollout_embedding", False))
        rollout_embedding_num_blocks = int(data.meta_info.get("rollout_embedding_num_blocks", 8))
        select_keys = ["responses", "input_ids", "attention_mask", "position_ids"]
        if calculate_rollout_embedding:
        ...
        topk_ids_lst = []
        topk_log_probs_lst = []
        rollout_embedding_lst = []

        for micro_batch in micro_batches:
            micro_batch = micro_batch.to(get_device_id())
            model_inputs = {**micro_batch.batch, **micro_batch.non_tensor_batch}
            with torch.no_grad():
                entropy, log_probs, topk_ids, topk_log_probs, rollout_embedding = self._forward_micro_batch(
                    model_inputs, temperature=temperature, calculate_entropy=calculate_entropy, top_k=top_k,
                    calculate_rollout_embedding=calculate_rollout_embedding,
                    rollout_embedding_num_blocks=rollout_embedding_num_blocks,
                )
            # Keep on GPU to avoid expensive CPU-GPU transfer for large top-k
            ...
            if top_k > 0:
                ...
                topk_log_probs_lst.append(topk_log_probs)
            if calculate_rollout_embedding:
                rollout_embedding_lst.append(rollout_embedding)
        ...
        if top_k > 0:
            ...
            topk_log_probs_tensor = torch.concat(topk_log_probs_lst, dim=0)
        rollout_embeddings = torch.concat(rollout_embedding_lst, dim=0) if calculate_rollout_embedding else None
        ...
        if use_dynamic_bsz:
            ...
            if top_k > 0:
                ...
                topk_log_probs_tensor = restore_dynamic_batch(topk_log_probs_tensor, batch_idx_list)
            if calculate_rollout_embedding:
                rollout_embeddings = restore_dynamic_batch(rollout_embeddings, batch_idx_list)

        if calculate_rollout_embedding:
            return log_probs, entropys, topk_ids_tensor, topk_log_probs_tensor, rollout_embeddings
    ...
    def update_policy(self, data: DataProto):
        ...
        if "prompt_filter_loss_weights" in data.batch.keys():
            select_keys.append("prompt_filter_loss_weights")

        if "rollout_weight" in data.batch.keys():
            select_keys.append("rollout_weight")
        ...
        for _ in range(self.config.ppo_epochs):
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    prompt_filter_loss_weights = model_inputs.get("prompt_filter_loss_weights", None)
                    rollout_weight = model_inputs.get("rollout_weight", None)
                    if rollout_weight is not None and rollout_weight.requires_grad:
                        raise RuntimeError("rollout_weight must be detached before actor optimization")
                    policy_objective_sequence_weights = None
                    for sequence_weights in (prompt_filter_loss_weights,):
                        if sequence_weights is not None:
                            token_weights = sequence_weights.unsqueeze(-1)
                            policy_objective_sequence_weights = token_weights if policy_objective_sequence_weights is None else policy_objective_sequence_weights * token_weights
                    full_objective_sequence_weights = None
                    for sequence_weights in (prompt_filter_loss_weights,):
                        if sequence_weights is not None:
                            token_weights = sequence_weights.unsqueeze(-1)
                            full_objective_sequence_weights = token_weights if full_objective_sequence_weights is None else full_objective_sequence_weights * token_weights

                    entropy_coeff = self.config.entropy_coeff
                    token_entropy_weighting_enable = bool(self.config.get("token_entropy_weighting_enable", False))
                    token_entropy_weighting_window_size = int(self.config.get("token_entropy_weighting_window_size", 2))
                    token_entropy_weighting_epsilon = float(self.config.get("token_entropy_weighting_epsilon", 1e-6))
                    ...
                    calculate_entropy = entropy_coeff != 0 or token_entropy_weighting_enable
                    ...
                    if token_entropy_weighting_enable:
                        if entropy is None:
                            raise RuntimeError("Token entropy weighting requires current-policy token entropy")
                        if entropy_coeff == 0:
                            entropy = entropy.detach()
                        token_weighting = compute_token_entropy_weighting(
                            token_entropies=entropy, response_mask=response_mask,
                            window_size=token_entropy_weighting_window_size, epsilon=token_entropy_weighting_epsilon,
                        )
                        token_loss_weights = token_weighting["loss_multiplier"]
                        with torch.no_grad():
                            valid_token_weights = token_loss_weights[response_mask.bool()]
                            valid_counts = response_mask.sum(dim=-1).float()
                            probabilities = token_weighting["normalized_weight"]
                            sequence_ess = probabilities.square().sum(dim=-1).clamp_min(torch.finfo(probabilities.dtype).tiny).reciprocal()
                            valid_sequences = valid_counts > 0
                            if valid_token_weights.numel() > 0:
                                micro_batch_metrics["actor/token_entropy_weight_mean"] = valid_token_weights.mean().item()
                                micro_batch_metrics["actor/token_entropy_weight_min"] = valid_token_weights.min().item()
                                micro_batch_metrics["actor/token_entropy_weight_max"] = valid_token_weights.max().item()
                            if valid_sequences.any():
                                micro_batch_metrics["actor/token_entropy_ess_fraction"] = (sequence_ess[valid_sequences] / valid_counts[valid_sequences]).mean().item()
                                positive_peak_fraction = ((token_weighting["soft_score"] > 0) & response_mask.bool()).sum(dim=-1).float() / valid_counts.clamp_min(1)
                                micro_batch_metrics["actor/token_entropy_positive_peak_fraction"] = positive_peak_fraction[valid_sequences].mean().item()
                    ...
                    if policy_objective_sequence_weights is not None:
                        # Apply sequence multipliers after clipping/OPD Top-K reduction, before aggregation.
                        rollout_is_weights = policy_objective_sequence_weights if rollout_is_weights is None else rollout_is_weights * policy_objective_sequence_weights
                    if token_loss_weights is not None:
                        # The policy loss functions apply these positive weights to each token objective after
                        # PPO clipping (and after OPD Top-K reduction) but before token/sequence aggregation.
                        rollout_is_weights = token_loss_weights if rollout_is_weights is None else rollout_is_weights * token_loss_weights
                    ...
                    if rollout_weight is not None and loss_mode != "vanilla":
                        raise ValueError("rollout_weight currently requires actor.policy_loss.loss_mode=vanilla")
                    ...
                    pg_loss, pg_metrics = policy_loss_fn(
                        ...
                        sequence_weights=rollout_weight,
                    )
                    micro_batch_metrics.update(pg_metrics)

                    if entropy_coeff != 0:
                        entropy_loss_mat = entropy if full_objective_sequence_weights is None else entropy * full_objective_sequence_weights
                        entropy_loss = agg_loss(loss_mat=entropy_loss_mat, loss_mask=response_mask, loss_agg_mode=loss_agg_mode)
                    ...
                    if self.config.use_kl_loss:
                        ...
                        # Hard prompt filtering removes filtered prompts from every optimization term.
                        kld = kl_penalty(
                            logprob=log_prob, ref_logprob=ref_log_prob, kl_penalty=self.config.kl_loss_type
                        )
                        if full_objective_sequence_weights is not None:
                            kld = kld * full_objective_sequence_weights
